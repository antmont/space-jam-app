...
function displayPlayerBench() {
    ...
    for (const playerName of playerMap.keys()) {
...
function displayPlayerCards() {
    ...
    // specific quarter.
    for (const [playerName, playerStats] of playerMap.entries()) {
...
function updateCardsInGame() {
    ...
    for (let playerIndex = 0; i < currentPlayers.length; playerIndex++) {
        // Get the name of the player
        const playerName = currentPlayers[playerIndex].id;
...
function startNextQuarter() {
    ...
    document.getElementById('start').innerText = 'Q' + (currentQuarter + 1) + ' in progress';
